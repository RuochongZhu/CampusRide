        ...
        <div class="w-full lg:w-2/3">
          <div class="bg-white rounded-xl shadow-lg p-4 md:p-6 mb-4 md:mb-6">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              <button
                :class="[
                  ...
                  rideFilterTab === 'all' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                ]"
              ...
              <button
                :class="[
                  'px-3 py-1.5 text-xs md:text-sm rounded-full border transition-all',
                  rideFilterTab === 'offer' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                ]"
                @click="rideFilterTab = 'offer'"
              >
                Driver Offers ({{ filterCounts.offer }})
              </button>
              <button
                :class="[
                  'px-3 py-1.5 text-xs md:text-sm rounded-full border transition-all',
                  rideFilterTab === 'request' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'
                ]"
            ...
            <div v-else class="space-y-3 md:space-y-4">
              <div v-for="ride in filteredRides" :key="ride.id" class="border border-gray-200 rounded-lg p-3 md:p-4 hover:shadow-md transition-shadow cursor-pointer" @click="viewRideDetails(ride)">
                <div class="flex flex-col gap-3 md:gap-4">
                  ...
                  <div class="flex items-start gap-3 md:hidden">
                    ...
                    <div class="flex-1 min-w-0">
                      <p class="text-xs font-semibold text-gray-700 truncate">{{ getRideDisplayTitle(ride) }}</p>
                      ...
                      <p class="text-xs text-gray-500 mt-1">{{ getDepartureCountdown(ride.departure_time) }}</p>
                      <div class="flex items-center justify-between mt-2">
                        <span class="text-lg font-bold text-red-600">${{ ride.price_per_seat }}</span>
                        <span class="text-xs bg-gray-100 rounded-full px-2 py-0.5">{{ getRemainingSeats(ride) }} seats</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="hidden md:flex md:flex-row gap-4">
                    ...
                    <div class="md:w-2/4">
                      <div class="flex items-start mb-3">
                        ...
                        <div>
                          <p class="font-semibold mb-1">{{ getRideDisplayTitle(ride) }}</p>
                      ...
                      <div class="flex flex-wrap gap-2">
                        <div class="bg-gray-100 rounded-full px-3 py-1 text-sm flex items-center">
                          <team-outlined class="mr-1" />{{ getRemainingSeats(ride) }} seats available
                        </div>
                        <div v-if="isRequestRide(ride)" class="bg-blue-100 text-blue-700 rounded-full px-3 py-1 text-sm flex items-center">
                          Request
                        </div>
                        <div class="bg-green-100 text-green-700 rounded-full px-3 py-1 text-sm flex items-center">
                          {{ formatRideStatus(ride.status) }}
                        </div>
                        <div class="bg-gray-100 text-gray-700 rounded-full px-3 py-1 text-sm flex items-center">
                          {{ getDepartureCountdown(ride.departure_time) }}
                        </div>
                      </div>
                    </div>
                    <div class="md:w-1/4 flex flex-col justify-between">
                      <div>
                        ...
                        <p class="text-sm text-gray-500">{{ getRidePriceLabel(ride) }}</p>
                      </div>
                      <!-- Dynamic button based on booking status -->
                      <template v-if="isOwnRide(ride)">
                        <button
                          class="bg-gray-500 text-white py-2 px-4 rounded-md font-medium mt-4 cursor-not-allowed opacity-75"
                          disabled
                        >
                          Your Ride
                        </button>
                  ...
                  <div class="md:hidden">
                    <template v-if="isOwnRide(ride)">
                      <button class="w-full bg-gray-500 text-white py-2 rounded-md text-sm font-medium cursor-not-allowed opacity-75" disabled>
                        Your Ride
                      </button>
                    </template>
                    <template v-else-if="ride.is_booked_by_user">
              ...
              <div v-if="filteredRides.length === 0" class="text-center py-6 md:py-8">
                <inbox-outlined style="font-size: 36px" class="text-gray-300 mb-3 md:mb-4 md:text-5xl" />
                <p class="text-gray-500 text-sm md:text-base">{{ ridesEmptyMessage }}</p>
              </div>
            </div>
          ...
          <div class="bg-white rounded-xl shadow-lg p-4 md:p-6">
            ...
            <div v-else-if="myTripsTab === 'driver'" class="space-y-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <p class="font-semibold text-sm md:text-base truncate">{{ getRideDisplayTitle(ride) }}</p>
            ...
            <div v-else class="space-y-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <p class="font-semibold text-sm md:text-base truncate">{{ getRideDisplayTitle(booking.ride) || 'Ride' }}</p>
          ...
          <div class="space-y-4">
            <div>
              ...
              <a-input-number
                ...
                :min="1"
                :max="Math.max(getRemainingSeats(selectedRide), 1)"
                class="w-full"
              />
            </div>
            <div>
      ...
      >
        <div v-if="selectedRide" class="space-y-4">
          ...
          <div class="border-t pt-4">
            ...
            <p><strong>Title:</strong> {{ getRideDisplayTitle(selectedRide) }}</p>
            ...
            <p><strong>Available Seats:</strong> {{ getRemainingSeats(selectedRide) }}</p>
            <p><strong>Price per Seat:</strong> ${{ selectedRide.price_per_seat }}</p>
            <p v-if="isRequestRide(selectedRide)"><strong>Type:</strong> Ride Request</p>
          </div>
          ...
          <template v-else>
            ...
            >
              {{ getRideDetailActionText(selectedRide) }}
            </button>
          </template>
        </div>
      </a-modal>
...
const currentUserId = computed(() => authStore.userId || getStoredUserId());
...
const initGoogleMaps = async () => {
  ...
  try {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=${import.meta.env.VITE_GOOGLE_MAPS_API_KEY}&libraries=places&language=en&region=US&callback=initMap&loading=async`;
    script.async = true;
    script.defer = true;
...
const openBookingModal = (ride) => {
  ...
  if (isOwnRide(ride)) {
    notification.info({
      ...
      description: 'You cannot book seats on a trip you posted.'
    });
    return;
  }

  if (getRemainingSeats(ride) <= 0) {
    notification.warning({
      message: 'Ride is full',
      description: 'No seats are currently available for this ride.'
    });
    return;
  }
...
const confirmBooking = async () => {
  ...
  const remainingSeats = getRemainingSeats(selectedRide.value);
  if (remainingSeats <= 0) {
    notification.error({
      message: 'Ride is full',
      description: 'Please refresh the list and choose another ride.'
    });
    return;
  }

  if (bookingForm.value.seats > remainingSeats) {
    notification.error({
      message: 'Seat count changed',
      description: `Only ${remainingSeats} seat(s) are available now.`
    });
    bookingForm.value.seats = remainingSeats;
    return;
  }
...
const getRideDisplayTitle = (rideOrTitle) => {
  const rawTitle = typeof rideOrTitle === 'string' ? rideOrTitle : rideOrTitle?.title;
  if (!rawTitle) return '';
  return rawTitle.replace(REQUEST_TITLE_PREFIX_REGEX, '').trim();
};

const isRequestRide = (ride) => {
  const rideType = String(ride?.ride_type || ride?.rideType || '').toLowerCase();
  if (rideType === 'request') return true;
  return REQUEST_TITLE_PREFIX_REGEX.test(ride?.title || '');
};

const getRemainingSeats = (ride) => {
  if (!ride) return 0;
  if (typeof ride.remaining_seats === 'number') return Math.max(ride.remaining_seats, 0);
  if (typeof ride.available_seats === 'number') return Math.max(ride.available_seats, 0);
  return 0;
};

const isOwnRide = (ride) => {
  const rideDriverId = ride?.driver_id || ride?.driver?.id;
  return Boolean(currentUserId.value && rideDriverId && currentUserId.value === rideDriverId);
};

const matchesRideFilter = (ride, tab) => {
  if (tab === 'offer') return !isRequestRide(ride);
  if (tab === 'request') return isRequestRide(ride);
  if (tab === 'available') return getRemainingSeats(ride) > 0;
  return true;
};

const filterCounts = computed(() => {
  const rides = availableRides.value || [];
  return {
    all: rides.length,
    offer: rides.filter(ride => matchesRideFilter(ride, 'offer')).length,
    request: rides.filter(ride => matchesRideFilter(ride, 'request')).length,
    available: rides.filter(ride => matchesRideFilter(ride, 'available')).length
  };
});

const filteredRides = computed(() => {
  const rides = availableRides.value || [];
  return rides.filter(ride => matchesRideFilter(ride, rideFilterTab.value));
});

const ridesEmptyMessage = computed(() => {
  const tab = rideFilterTab.value;
  if (tab === 'offer') return 'No driver offers found on this page.';
  if (tab === 'request') return 'No rider requests found on this page.';
  if (tab === 'available') return 'No rides with available seats right now.';
  return 'No rides available. Be the first to post!';
});

const getRidePriceLabel = (ride) => {
  return isRequestRide(ride) ? 'max per seat' : 'per person';
};

const getRidePrimaryActionText = (ride) => {
  return isRequestRide(ride) ? 'Join Request' : 'Book Seat';
};

const getRideDetailActionText = (ride) => {
...
const getDepartureCountdown = (departureTime) => {
  ...
  if (!departure.isValid()) return 'Departure time pending';

  const now = dayjs();
  const diffMinutes = departure.diff(now, 'minute');
  if (diffMinutes <= 0) return 'Departing soon';
  if (diffMinutes < 60) return `Departs in ${diffMinutes}m`;

  const diffHours = departure.diff(now, 'hour');
  if (diffHours < 24) return `Departs in ${diffHours}h`;

  const diffDays = departure.diff(now, 'day');
  return `Departs in ${diffDays}d`;
};
