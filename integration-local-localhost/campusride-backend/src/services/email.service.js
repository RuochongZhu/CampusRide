...
const EMAIL_DISABLED = process.env.ENABLE_EMAIL_SENDING === 'false';
...
export const sendVerificationEmail = async (email, token) => {
  try {
    ...
    if (EMAIL_DISABLED) {
      console.log(`📧 Email sending disabled; skip verification email to ${email}`);
      return { success: true, emailId: 'local-disabled' };
    }
...
export const sendPasswordResetEmail = async (email, token) => {
  try {
    ...
    if (EMAIL_DISABLED) {
      console.log(`📧 Email sending disabled; skip password reset email to ${email}`);
      return { success: true, emailId: 'local-disabled' };
    }
