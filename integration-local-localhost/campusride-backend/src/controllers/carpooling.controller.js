...
export const bookRide = async (req, res, next) => {
  try {
    ...
    const rideTitleForDisplay = getRideDisplayTitle(ride.title);
    ...
    await notificationService.sendNotification({
      ...
      content: `${passengerName} booked ${seatsBooked} seat(s) on "${rideTitleForDisplay}".`,
      data: {
        rideId,
    ...
    await notificationService.sendNotification({
      ...
      content: `You are confirmed for "${rideTitleForDisplay}".`,
      data: {
        rideId,
        bookingId: booking.id,
    ...
    await notificationService.sendNotification({
      ...
      content: `Payment confirmed for "${rideTitleForDisplay}" ($${Number(totalPrice).toFixed(2)}).`,
      data: {
        rideId,
        bookingId: booking.id,
        amount: totalPrice,
    ...
    await notificationService.sendNotification({
      ...
      content: `Payment received from ${passengerName} for "${rideTitleForDisplay}" ($${Number(totalPrice).toFixed(2)}).`,
      data: {
        rideId,
        bookingId: booking.id,
        passengerId: userId,
    ...
    await notificationService.sendNotification({
      ...
      content: `Please rate the driver for "${rideTitleForDisplay}". Tap stars to submit.`,
      data: {
        rideId,
    ...
    await notificationService.sendNotification({
      ...
      content: `Please rate ${passengerName} for "${rideTitleForDisplay}". Tap stars to submit.`,
      data: {
        rideId,
        rateeId: userId,
        roleOfRater: 'driver',
