...
export const marketplaceAPI = {
  ...
  // 评论相关
  // 获取商品评论
  getComments: (itemId, params = {}) => api.get(`/marketplace/items/${itemId}/comments`, { params }),
  ...
  // 点赞/取消点赞评论
  toggleLike: (commentId) => api.post(`/marketplace/comments/${commentId}/like`),

  // 点踩/取消点踩评论
  toggleDislike: (commentId) => api.post(`/marketplace/comments/${commentId}/dislike`),

  // 获取点赞状态
  getLikeStatus: (commentIds) => api.post('/marketplace/comments/like-status', { commentIds }),

  // 消息相关
  // 开始商品对话
  startConversation: (itemId, message) => api.post(`/marketplace/items/${itemId}/message`, { message }),

  // 发送消息
  sendMessage: (conversationId, message) => api.post(`/marketplace/conversations/${conversationId}/messages`, { message }),

  // 获取用户对话
  getConversations: (params = {}) => api.get('/marketplace/conversations', { params }),

  // 获取对话消息
  getConversationMessages: (conversationId, params = {}) => api.get(`/marketplace/conversations/${conversationId}/messages`, { params }),

  // 获取未读消息数量
  getUnreadCount: () => api.get('/marketplace/conversations/unread-count'),
};
...
export const uploadAPI = {
  ...
  // 删除文件
  deleteFile: (fileUrl) => api.delete('/upload', { data: { fileUrl } }),

  // 获取文件信息
  getFileInfo: (fileUrl) => api.get('/upload/info', { params: { fileUrl } }),
};
