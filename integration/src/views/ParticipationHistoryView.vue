  ...
  <div class="min-h-screen bg-[#EDEEE8] main-content pt-16">
    <div class="pt-8 pb-16 max-w-7xl mx-auto px-4">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-3xl font-bold text-[#333333]">参加历史</h1>
          <p class="text-sm text-gray-600 mt-1">查看您参与过的所有活动记录</p>
        </div>
        <a-button type="default" @click="() => $router.push('/activities')">
          <ArrowLeftOutlined /> 返回活动列表
        </a-button>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
        <a-radio-group v-model:value="historyType" button-style="solid" @change="loadHistory">
          <a-radio-button value="participated">我参加的活动</a-radio-button>
          <a-radio-button value="organized">我组织的活动</a-radio-button>
        </a-radio-group>
      </div>
      ...
      <div v-if="loading" class="bg-white rounded-lg shadow-sm p-12 text-center">
        <a-spin size="large" />
        <p class="mt-4 text-gray-500">加载历史记录中...</p>
      </div>
      ...
      <div v-else-if="activities.length === 0" class="bg-white rounded-lg shadow-sm p-12 text-center">
        ...
        <p class="text-sm text-gray-500 mb-4">
          {{ historyType === 'participated' ? '您还没有参加过任何活动' : '您还没有组织过任何活动' }}
        </p>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div class="flex items-center text-sm text-gray-600">
              ...
              <div>
                ...
                <div>{{ activity.current_participants || 0 }} / {{ activity.max_participants || '不限' }}</div>
              </div>
            </div>
          </div>
...
const loadHistory = async () => {
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.error?.message || '加载历史记录失败')
    }
  } catch (err) {
    console.error('Failed to load activity history:', err)
    error.value = err.response?.data?.error?.message || err.message || '加载历史记录失败'
