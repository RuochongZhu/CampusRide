...
const props = defineProps({
  ...
  roleOfRater: {
    type: String,
    required: true,
    validator: (value) => ['driver', 'passenger'].includes(value)
  }
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (error.response?.status === 400) {
      ...
      errorMessage.value = 'You are not authorized to rate this user for this trip.';
    } else if (error.response?.status === 404) {
      errorMessage.value = 'Trip or user not found.';
    }
  } finally {
